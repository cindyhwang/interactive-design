...
var xhr=jQuery.ajax({data:options.data,dataType:options.dataType||'json',type:options.type||'POST',url:options.url,success:function(data,status)
...
var self=this;this.startLoader();var r=doAjax({data:data,lock:jQuery(''+lockdiv)[0],url:this.url,dataType:responseType,onSuccess:function(data)
{self.stopLoader(data);for(var eachvar in data)
{this.response+=eachvar+": "+data[eachvar]+"\n";}
eval(returnobj+"."+returnfunc+"(data)");},onError:function(e){self.stopLoader();alert("Error: "+e);}});doAddAjax(r,this.module+"Request");},createLoaderDiv:function()
...
jQuery("#"+this.dialogId).dialog({width:this.width,height:this.height,title:"Alert",buttons:{"OK":function(){mmdialog_js.close(this.dialogId)}}});jQuery("#"+this.dialogId).html(message);jQuery("#"+this.dialogId).dialog("open");},disableDialog:function(dialogId){if(dialogId==undefined){jQuery("#"+this.dialogId).dialog("option","disabled",true);}
